...
fn get_engine() -> Engine {
    ...
    Engine::new(PulumiServiceImpl::new(
        native_pulumi_connector,
        in_preview,
    ))
}
...
pub extern "C" fn pulumi_get_output(custom_register_output_id: *mut CustomRegisterOutputId, field_name: *const c_char) -> *mut CustomOutputId {
    ...
    let output_id = unsafe {
        &(*custom_register_output_id).output_id.clone()
    };
...
#[no_mangle]
pub extern "C" fn pulumi_register_resource(pulumi_engine: *mut PulumiEngine, request: *const RegisterResourceRequest) -> *mut CustomRegisterOutputId {
    let pulumi_engine = unsafe {
        &mut *pulumi_engine
    };

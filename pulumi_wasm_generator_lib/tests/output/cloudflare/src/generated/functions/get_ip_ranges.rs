...
pub fn invoke() -> GetIpRangesResult {
    ...
    let request = register_interface::ResourceInvokeRequest {
        ...
        object: Vec::from([]),
        results: vec![
            register_interface::ResultField { name : "chinaIpv4CidrBlocks".into() },
            register_interface::ResultField { name : "chinaIpv6CidrBlocks".into() },
            register_interface::ResultField { name : "cidrBlocks".into() },
            register_interface::ResultField { name : "id".into() },
            register_interface::ResultField { name : "ipv4CidrBlocks".into() },
            register_interface::ResultField { name : "ipv6CidrBlocks".into() },
        ],
    };
    ...
    let mut hashmap: HashMap<String, _> = o
        ...
        .collect();
    GetIpRangesResult {
        china_ipv4_cidr_blocks: into_domain(
            hashmap.remove("chinaIpv4CidrBlocks").unwrap(),
        ),
        china_ipv6_cidr_blocks: into_domain(
            hashmap.remove("chinaIpv6CidrBlocks").unwrap(),
        ),
        cidr_blocks: into_domain(hashmap.remove("cidrBlocks").unwrap()),
        id: into_domain(hashmap.remove("id").unwrap()),
        ipv4_cidr_blocks: into_domain(hashmap.remove("ipv4CidrBlocks").unwrap()),
        ipv6_cidr_blocks: into_domain(hashmap.remove("ipv6CidrBlocks").unwrap()),
    }
}

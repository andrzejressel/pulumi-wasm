...
fn msgpack_to_protobuf(v: &MsgpackValue) -> Result<ProtobufValue> {
    ...
    let result = match v {
        ...
        MsgpackValue::Integer(i) => prost_types::Value {
            kind: Option::from(prost_types::value::Kind::NumberValue(
                i.as_f64()
        ...
        MsgpackValue::Map(map) => {
            ...
            prost_types::Value {
                kind: Option::from(prost_types::value::Kind::StructValue(prost_types::Struct {
                    fields: map
                        .iter()
                        .zip(keys)
                        .map(|((_, v), k)| (k, msgpack_to_protobuf(v).unwrap()))
                        .collect(),
                })),
            }
        }

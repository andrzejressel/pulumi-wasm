...
fn main() {
    ...
    for provider in &providers {
        ...
        let schema = String::from_utf8(schema_output.stdout)
            .expect("Invalid UTF-8 in pulumi output");
...
fn update_cargo_toml(providers: &[Provider]) {
    ...
    for provider in providers {
        replacement.push_str(&format!("    \"providers/pulumi_wasm_provider_{}\",\n", provider.name));
        replacement.push_str(&format!("    \"providers/pulumi_wasm_provider_{}_rust\",\n", provider.name));
    }
...
fn update_justfile(providers: &[Provider]) {
    let content = fs::read_to_string("justfile")
        .expect("Failed to read justfile");
    ...
    fs::write("justfile", new_content)
        .expect("Failed to write to justfile");
}

fn replace_between_markers(source: &str, start_marker: &str, end_marker: &str, replacement: &str) -> String {
    let start_index = source.find(start_marker).expect("Start marker not found in source");
    let end_index = source.find(end_marker).expect("End marker not found in source");

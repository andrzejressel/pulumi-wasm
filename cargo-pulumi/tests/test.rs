...
fn run_from_subdirectory() -> Result<(), Error> {
    ...
    let (mut store, plugin) = create_engine(
        dir.join("targets/target/run_from_subdirectory/wasm32-wasip1/debug/pulumi/main.wasm")
            .to_str()
            .unwrap(),
    )?;
    let result = plugin
...
fn run_from_main_directory() -> Result<(), Error> {
    ...
    let (mut store, plugin) = create_engine(
        dir.join("targets/target/run_from_main_directory/wasm32-wasip1/debug/pulumi/main.wasm")
            .to_str()
            .unwrap(),
    )?;
    let result = plugin
